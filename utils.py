...
def get_nnet(name, **kwargs):
    if name == 'uvit':
        ...
        from libs.uvit_tiny import UViT_tiny
        return UViT_tiny(**kwargs)
    elif name == 'uvit_transformer':
        from libs.uvit_tiny import UViT_tiny_Transformer
        return UViT_tiny_Transformer(**kwargs)
    elif name == 'uvit_mlp':
        from libs.uvit_tiny import UViT_tiny_MLP
        return UViT_tiny_MLP(**kwargs)
    else:
...
def sample2dir(accelerator, path, n_samples, mini_batch_size, sample_fn, unpreprocess_fn=None):
    def save_image_(sample, path):
        sample = sample.permute(1, 2, 0).cpu().numpy()
        np.save(path,sample[:,:,0])
    os.makedirs(path, exist_ok=True)
    idx = 0
    ...
    for _batch_size in tqdm(amortize(n_samples, batch_size), disable=not accelerator.is_main_process, desc='sample2dir'):
        ...
        if accelerator.is_main_process:
            for sample in samples:
                save_image_(sample, os.path.join(path, f"{idx}"))
                idx += 1

def sample2dir_2(path, n_samples, mini_batch_size, sample_fn, unpreprocess_fn=None):
    def save_image_(sample, path):
        sample = sample.permute(1, 2, 0).cpu().numpy()
        np.save(path,sample[:,:,0])
    os.makedirs(path, exist_ok=True)
    ...
    for _batch_size in tqdm(amortize(n_samples, batch_size), desc='sample2dir'):
        # samples = unpreprocess_fn(sample_fn(mini_batch_size))
